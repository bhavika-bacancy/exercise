		...
		print "Category \t Item \t\t Price \t Quantity\n"
				...
				@bill.each do |product|
					temp_array.each do |t|
						if product[:Category] == t[:Category] && product[:Item] == t[:Item]
							t[:Quantity]=t[:Quantity] + product[:Quantity]
							is_repeat=true
						end
					end
					if is_repeat == false
				...
				temp_array.each do |product|
					print "#{product[:Category]} \t #{product[:Item]} \t\t #{product[:Price]} \t #{product[:Quantity]}\n"
				end
				# @bill.each do |product|
				# 	@repeat.push(product)
				# 	@repeat.each do |i|
				# 		if i[:Category] == product[:Category] && i[:Item] == product[:Item]
				# 			product[:Quantity]= i[:Quantity] + product[:Quantity]
